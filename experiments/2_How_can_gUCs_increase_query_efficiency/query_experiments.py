...
def main():
    ...
    for scaling in range(scaling_times + 1):
        ...
        # query with index
            ...
            query = "PROFILE MATCH (ad:ActorAndDirector) WHERE ad.name IS NOT NULL AND ad.bornIn IS NOT NULL RETURN ad.name"
            current_db_hits, current_time = show_query_details(new_db, query)
            db_hits[-1][1].append(current_db_hits)
            times[-1][1].append(current_time)


        # delete nodes, drop new label and drop index to restart experiment for bigger scaling factor
    ...
    filename = "Query_results_" + str(factor*scaling) + "_" + str(today) + "---" + str(current) + ".xlsx"
    ...
    content = [["Query: factors 1 to " + str(factor * scaling) + " in steps of " + str(factor), "",""]]
    for index in range(scaling_times):
